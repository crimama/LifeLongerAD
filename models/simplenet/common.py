...
class NetworkFeatureAggregator(torch.nn.Module):
    ...
    def __init__(self, backbone, layers_to_extract_from, device, train_backbone=False):
        ...
        Args:
            backbone: torchvision.model or VitBackbone
            layers_to_extract_from: [list of str] for CNN or [list of int] for ViT
        """
        self.layers_to_extract_from = layers_to_extract_from
        self.backbone = backbone
        self.device = device
        self.train_backbone = train_backbone
        self.is_vit = hasattr(backbone, 'is_vit') and backbone.is_vit
    ...
    def forward(self, images, eval=True):
        if self.is_vit:
            if self.train_backbone and not eval:
                ...
                with torch.no_grad():
                    features = self.backbone(images, layers_to_extract=self.layers_to_extract_from)
            return {f'layer{i+1}': feat for i, feat in enumerate(features)}
        else:
            self.outputs.clear()
            if self.train_backbone and not eval:
    ...
    def feature_dimensions(self, input_shape):
        ...
        return [_output[layer].shape[1] for layer in 
                ([f'layer{i+1}' for i in range(len(self.layers_to_extract_from))] if self.is_vit 
                 else self.layers_to_extract_from)]
