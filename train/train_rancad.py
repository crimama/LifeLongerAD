...
def train(model, dataloader, testloader, optimizer, scheduler, accelerator, log_interval: int, epoch, epochs, savedir, cfg, drift_monitor) -> dict:
    ...
    def log_training_info(step, accelerator, dataloader, epoch, epochs,
                      losses_m, feature_losses_m, svd_losses_m, ranpac_ce_loss,
        ...
        _logger.info(
            ...
            'RanPAC Loss: {ranpac_ce_loss:>6.4f} | '
            'LR: {lr:.3e} | '
            ...
            'Data: {data_time_avg:.3f}s'.format(
                ...
                ranpac_ce_loss=ranpac_ce_loss.val,
    ...
    for step, (images, labels, class_labels) in enumerate(dataloader):
        ...
        ce_losses_m.update(loss['ranpac_ce_loss'])
...
def fit(
    ...
    # Set for RanPAC 
    num_new_classes_per_task = [len(k) for k,v in loader_dict.items()]
    ...
    for n_task, (current_class_name, class_loader_dict) in enumerate(loader_dict.items()):
        ...
        # Init RanPAC Continual state 
        num_new_classes = num_new_classes_per_task[n_task]
        from datasets.mvtecad import class_label_mapping
        idx_new_class_label = [class_label_mapping[name] for name in list(loader_dict.keys())[n_task]]
        model.reconstruction.update_continual_state(n_task, idx_new_class_label)
